...
const Overlay = ({show , close}) =>{
  ...
  const submitLogin = async ele => {
    ...
    await axios.post('https://akarat.develocity.app/api/auth/login', loginData, options).then(result => {
      console.log('prekoo', result.email);
    ...
    return(
      ...
      >
        {({ isSubmitting }) => (
          ...
          <div className={styles.inputBlock}>
            <Field className={styles.inputField} value={email} onChange={(e)=> setEmail(e.target.value)} placeholder="enter your email" type="email" name="email" />
          ...
          <div className={styles.inputBlock}>
            <Field className={styles.inputField} value={password} onChange={(e)=> setPassword(e.target.value)} placeholder="enter your password" type="password" name="password" />
      ...
      <li><button><img src={google} className={styles.google} alt="google icon" /></button></li>
      <li><button><FaFacebookF className={styles.fbIcon}/></button></li>
     
      <li><button onClick={showPhone}><BsTelephoneFill/></button>
      <LoginMobile show={showLoginPhone} close={close}/>
      </li>
      </ul>
      <p className={styles.text2}>by continuing , you are accepting <Link className={styles.link} to="/terms">real state terms of use</Link> and <Link className={styles.link} to="/policy">privacy policy</Link></p>
        </div>
    )
}
