...
const SellHomeContent = () => {
  ...
  const handleSubmitAdv = async (data, ele) => {
    ...
    const AdvData = {
      ...
      images: ["D:home-apphome-appsrcassets", "D:home-apphome-appsrcassets"],
      lat: lat,
      lng: lng,
      city_id: 1,
    };
    ...
    await axios
      .post("https://akarat.develocity.app/api/advertisements", AdvData, formData, options)
      .then((response) => {
      ...
      <form enctype="multipart/form-data" method="POST" onSubmit={handleSubmit(handleSubmitAdv)}>
        ...
        <div className={styles.radioButtons}>
          <div className={styles.radioBtnBlock}>
            ...
            <label onClick={handleRental} for="rent">
              Rent
            </label>
          </div>
          ...
          <div className={styles.radioBtnBlock}>
            ...
            <label onClick={handleRemoveRental} for="sell">
              Sell
            </label>
          </div>
        </div>

        {show && <Rental show={show} />}
          ...
          <div className={styles.part}>
            {sell?.data?.categories?.map(function (data, idx) {
              if (idx + 1 == categoryId) {
                ...
                return [
                  <label className={` ${styles.label} ${dataOptions.includes("bedrooms") ? null : styles.hide}`}>
                    Bedrooms
                  </label>,
                  <input
                    onChange={(e) => setBedrooms(e.target.value)}
                    className={`${styles.inputField} ${dataOptions.includes("bedrooms") ? null : styles.hide}`}
                  ...
                  <input
                    ...
                    placeholder="Type Number Of Bedrooms"
                  />,
                  <label className={` ${styles.label} ${dataOptions.includes("bathrooms") ? null : styles.hide}`}>
                    Bathrooms
                  </label>,
                  <input
                    onChange={(e) => setBathrooms(e.target.value)}
                    className={`${styles.inputField} ${dataOptions.includes("bathrooms") ? null : styles.hide}`}
                    placeholder="Type Number Of Bathrooms"
                  />,
                  <label className={` ${styles.label} ${dataOptions.includes("floor") ? null : styles.hide}`}>
                    Floor
                  </label>,
                  <input
                    onChange={(e) => setFloor(e.target.value)}
                    className={`${styles.inputField} ${dataOptions.includes("floor") ? null : styles.hide}`}
                    placeholder="Type Number Of Floor"
                  />,
                  <label className={` ${styles.label} ${dataOptions.includes("furnished") ? null : styles.hide}`}>
                    Furnished
                  </label>,
                  <div className={`${styles.radioButtons} ${dataOptions.includes("furnished") ? null : styles.hide}`}>
                    <div className={styles.radioBtnBlock}>
                      ...
                      <label onClick={handleFurnished} for="true">
                        Yes
                      </label>
                    </div>
                    <div className={styles.radioBtnBlock}>
                      <input type="radio" id="false" name="furnished" value="false" /> {" "}
                      <label onClick={handleNotFurnished} for="false">
                        No
                      </label>
                    </div>
                  </div>,
                  <label className={` ${styles.label} ${dataOptions.includes("condition") ? null : styles.hide}`}>
                    Condition
                  </label>,
                  ...
                  <label className={` ${styles.label} ${dataOptions.includes("type") ? null : styles.hide}`}>
                    Type
                  </label>,
                  ...
                  <label className={` ${styles.label} ${dataOptions.includes("equipped") ? null : styles.hide}`}>
                    Equipped
                  </label>,
        ...
        <div className={styles.cards}>
          {sell?.data?.categories?.map(function (data, idx) {
            ...
            if (idx + 1 == categoryId) {
              let title = (
                <h2 className={` ${dataProperty ? null : styles.hide} ${styles.secondTitle}`}>Property Amentities</h2>
              );
                ...
                <div className={styles.checkbox}>
                  <label className={styles.checkboxWrapper}>
                    <input
                      onChange={(e) => {
                        if (e.target.checked) {
                          setPropertyAmin([...propertyAmin, parseInt(e.target.value)]);
                        } else {
                          setPropertyAmin([...propertyAmin]);
                        }
                      }}
                      type="checkbox"
        ...
        <div className={styles.cards}>
          {sell?.data?.categories?.map(function (data, idx) {
            ...
            if (idx + 1 == categoryId) {
              let title2 = (
                <h2 className={` ${dataCommunity ? null : styles.hide} ${styles.secondTitle}`}>Community Amentities</h2>
              );
                ...
                <div className={styles.checkbox}>
                  <label className={styles.checkboxWrapper}>
                    <input
                      onChange={(e) => {
                        if (e.target.checked) {
                          setCommunityAmin([...communityAmin, parseInt(e.target.value)]);
                        } else {
                          setCommunityAmin([...communityAmin]);
                        }
                      }}
                      type="checkbox"
                      className={styles.checkboxInput}
        ...
        <div className="input-group mb-3">
          <div className="input-group-prepend">
            <span className={`${styles.price} input-group-text`} id="basic-addon1">
              USD
            </span>
          </div>
          <input
            ...
            type="text"
            className={`${styles.inputField} ${styles.formControl} form-control`}
        ...
        <div className={styles.check}>
          <input
            onClick={(e) => seNegotiable(!negotiable)}
            type="checkbox"
            id="negotiable"
            name="negotiable"
            value="negotiable"
...
/>*/}
        ...
        <button className={styles.advBtn} onClick={handleSubmitAdv} type="submit">
